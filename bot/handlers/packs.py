...
async def _ask_for_prefix(message: Message, user_id: int, repo: PackRepo, bot_name: str,
                          t: Translator, source: str | None = None) -> None:
    await repo.enter(user_id, states.PREFIX, source)
...
async def _build_pack(message: Message, user_id: int, base: str,
    ...
    try:
        ...
        await repo.enter(user_id, states.PREFIX)
...
async def _start_collecting(message: Message, user_id: int, title: str,
    ...
    await repo.enter(user_id, states.COLLECTING)
...
async def _restore(message: Message, status: Message, user_id: int, restored,
                   repo: PackRepo, packs: PackManager, t: Translator) -> None:
    into = await repo.open_pack(user_id)
...
async def plain_text(message: Message, bot: Bot, queue: JobQueue, repo: PackRepo,
    ...
    if here.state == states.NAMING:
        if here.target:
            ...
            me = await bot.me()
            await _ask_for_prefix(message, user_id, repo, me.username, t, here.target)
            return
        await _start_collecting(message, user_id, text, repo, t)
        return

    if here.state == states.SOURCE:
        await _take_source(message, user_id, text, repo, packs, t)
        return

    if here.state == states.PREFIX:
        me = await bot.me()
        if not await _usable_prefix(message, user_id, text, me.username, repo, packs, t):
            return
        if here.target:
            await _copy_pack(message, user_id, text, here.target, repo, packs, t)
        else:
            await _build_pack(message, user_id, text, repo, packs, t)
        return

    if here.state == states.CONFIRMING:
        await _settle_deletion(message, user_id, here.target, text, repo, packs, t)
        return

    if here.state == states.RETAGGING:
        if not is_emoji(text):
            await message.answer(t("emoji.only_plain"))
            return
        emoji = "".join(split_emoji(text))
        try:
            await packs.retag(here.sticker, emoji)
        except Exception as exc:
            log.warning("could not retag a sticker: %s", exc)
            await message.answer(t("emoji.change_failed"))
            return
        await repo.enter(user_id, states.PICKING, here.target)
        await drop_preview(message, user_id)
        await message.answer(t("emoji.changed", emoji=emoji))
        return
...
async def pack_menu(callback: CallbackQuery, repo: PackRepo, packs: PackManager,
    ...
    if action == "zip":
        ...
        await _render(callback, *_menu_view(pack, count, await _filling(user_id, pack, repo), t))
        return

    if action == "drop":
        await repo.enter(user_id, states.CONFIRMING, pack.name)
        await _render(callback, *_confirm_view(pack, t))
        return

    # opening the menu of a pack being filled leaves that alone, any other state ends here
    open_now = await _filling(user_id, pack, repo)
    if not open_now and action == "open":
        await repo.leave(user_id)
    await _render(callback, *_menu_view(pack, count, open_now, t))


async def _filling(user_id: int, pack, repo: PackRepo) -> bool:
    here = await repo.session(user_id)
    return here.state == states.FILLING and here.target == pack.name
...
async def editing_a_pack(message: Message, repo: PackRepo) -> bool:
    ...
    assert message.from_user
    here = await repo.session(message.from_user.id)
    return bool(here.state in states.STICKER_MODE and message.sticker
                and message.sticker.set_name == here.target)
...
async def pick_by_sticker(message: Message, repo: PackRepo, packs: PackManager,
    ...
    here = await repo.session(user_id)
    name = here.target
    pack = await repo.find(user_id, name) if name else None
    if pack is None:
        await repo.leave(user_id)
        await message.answer(t("delete.gone"))
        return
...
async def edit_menu(callback: CallbackQuery, repo: PackRepo, packs: PackManager,
    ...
    if action == "list":
        await repo.enter(user_id, states.PICKING, pack.name)
        await callback.answer()
        await _render(callback, *_sticker_list_view(pack, entries, t))
        return
    ...
    if action in ("pick", "emoji"):
        if action == "emoji":
            await repo.enter(user_id, states.RETAGGING, pack.name, file_id)
        await callback.answer()
        if isinstance(callback.message, Message):
...
async def cmd_import(message: Message, command: CommandObject,
    ...
    if not raw:
        await repo.enter(user_id, states.SOURCE)
...
async def _take_source(message: Message, user_id: int, raw: str,
    ...
    await repo.enter(user_id, states.NAMING, source)
...
async def _copy_pack(message: Message, user_id: int, base: str, source: str,
                     repo: PackRepo, packs: PackManager, t: Translator) -> None:
    title = await repo.peek_pending(user_id) or base
